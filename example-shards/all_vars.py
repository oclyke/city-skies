...
import pysicgl
from hidden_shades.variables.types import BooleanVariable, IntegerVariable, FloatingVariable, OptionVariable, ColorSequenceVariable, StringVariable
...
def frames(layer):
    ...
    layer.variable_manager.declare_variable(
        FloatingVariable("float", 0.001)
    )
    layer.variable_manager.declare_variable(
        OptionVariable("option", "option 1", ["option 1", "option 2", "option 3"])
    )
    layer.variable_manager.declare_variable(
        ColorSequenceVariable("color", pysicgl.ColorSequence([0xFF0000FF]))
    )
    layer.variable_manager.declare_variable(
        StringVariable("string", "a frayed knot")
    )
    ...
    while True:
        ...
        print(f"string: {string}, color: {color}, option: {option}, float: {float}, integer: {integer}, boolean: {boolean}, coolBool: {coolBool}")
