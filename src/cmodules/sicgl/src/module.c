...
STATIC mp_obj_t allocate_memory(mp_obj_t obj) {
    // allocate memory
    // the amount of memory allocated is determined according to the type of the argument
    size_t pixels = 0;
    ...
    // allocate memory
    // not sure if this could be a memory leak... will the memory be freed when the bytearray is collected?
...
STATIC const mp_map_elem_t sicgl_globals_table[] = {
    ...
    // methods
    {MP_OBJ_NEW_QSTR(MP_QSTR_allocate_memory), (mp_obj_t)MP_ROM_PTR(&allocate_memory_obj)},
