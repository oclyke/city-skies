...
mp_obj_t scalar_field(size_t n_args, const mp_obj_t *args) {
  ...
  ColorSequence_obj_t* pysequence = color_sequence_from_obj(args[ARG_color_sequence]);
  ScalarField_obj_t* scalar_field = scalar_field_from_obj(args[ARG_scalar_field]);
  ...
  // apply the field
  ret = sicgl_scalar_field(&self->interface, field->screen, scalar_field->scalars, &sequence, pysequence->map);
  if (0 != ret) {
    mp_raise_OSError(ret);
  }
