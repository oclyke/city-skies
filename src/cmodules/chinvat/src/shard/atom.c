...
typedef struct _Shard_Atom_obj_t {
  mp_obj_base_t base;
  Shard_Atom def;
} Shard_Atom_obj_t;
...
STATIC void print(
  ...
  (void)kind;
  // Shard_Atom_obj_t* self = MP_OBJ_TO_PTR(self_in);
  mp_print_str(print, "Shard_Atom(");
  mp_print_str(print, ")");
}
...
STATIC mp_obj_t make_new(
    ...
    const mp_obj_t* args) {
  Shard_Atom_obj_t* self = m_new_obj(Shard_Atom_obj_t);
  self->base.type = &Shard_Atom_type;
...
const mp_obj_type_t Shard_Atom_type = {
    {&mp_type_type},
    .name = MP_QSTR_Shard_Atom,
    .print = print,
    .make_new = make_new,
