...
typedef struct _Shard_Variable_obj_t {
  mp_obj_base_t base;
  Shard_Variable def;
} Shard_Variable_obj_t;

STATIC void print(
    const mp_print_t* print, mp_obj_t self_in, mp_print_kind_t kind) {
  (void)kind;
  // Shard_Variable_obj_t* self = MP_OBJ_TO_PTR(self_in);
  mp_print_str(print, "Shard_Variable(");
  mp_print_str(print, ")");
}
...
STATIC mp_obj_t make_new(
    ...
    const mp_obj_t* args) {
  Shard_Variable_obj_t* self = m_new_obj(Shard_Variable_obj_t);
  self->base.type = &Shard_Variable_type;
...
const mp_obj_type_t Shard_Variable_type = {
    {&mp_type_type},
    .name = MP_QSTR_Shard_Variable,
    .print = print,
    .make_new = make_new,
