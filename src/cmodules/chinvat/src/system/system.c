...
typedef struct _System_SystemInfo_obj_t {
  mp_obj_base_t base;
  System_SystemInfo def;
} System_SystemInfo_obj_t;

STATIC void print(
    const mp_print_t* print, mp_obj_t self_in, mp_print_kind_t kind) {
  (void)kind;
  // System_SystemInfo_obj_t* self = MP_OBJ_TO_PTR(self_in);
  mp_print_str(print, "System_SystemInfo(");
  mp_print_str(print, ")");
}
...
STATIC mp_obj_t make_new(
    ...
    const mp_obj_t* args) {
  System_SystemInfo_obj_t* self = m_new_obj(System_SystemInfo_obj_t);
  self->base.type = &System_SystemInfo_type;
...
const mp_obj_type_t System_SystemInfo_type = {
    {&mp_type_type},
    .name = MP_QSTR_System_SystemInfo,
    .print = print,
    .make_new = make_new,
