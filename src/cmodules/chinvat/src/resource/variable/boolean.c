...
typedef struct _Boolean_obj_t {
  mp_obj_base_t base;
  Resource_Variable_Boolean def;
} Boolean_obj_t;

STATIC void print(
    const mp_print_t* print, mp_obj_t self_in, mp_print_kind_t kind) {
  (void)kind;
  // Boolean_obj_t* self = MP_OBJ_TO_PTR(self_in);
  mp_print_str(print, "Boolean(");
  mp_print_str(print, ")");
}
...
STATIC mp_obj_t make_new(
    ...
    const mp_obj_t* args) {
  Boolean_obj_t* self = m_new_obj(Boolean_obj_t);
  self->base.type = &Boolean_type;
...
const mp_obj_type_t Boolean_type = {
    {&mp_type_type},
    .name = MP_QSTR_Boolean,
    .print = print,
    .make_new = make_new,
